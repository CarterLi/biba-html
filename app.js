angular.module("BibaApp", ['ui.router', 'ui.bootstrap', 'angularFileUpload']).directive('emoji', function () { return ({
    priority: 10,
    restrict: 'A',
    link: function ($scope, $elem, $attrs) {
        $scope.$watch($attrs['ngBind'], function () { return window['emojify'].run($elem[0]); });
    }
}); }).directive('autolink', function () { return ({
    priority: 5,
    restrict: 'A',
    link: function ($scope, $elem, $attrs) {
        $scope.$watch($attrs['ngBind'], function () { return $elem.html(window['Autolinker'].link($elem.html(), { newWindow: true })); });
    }
    ...
    return {
        ...
        restrict: 'A',
        link: function ($scope, $elem, $attrs) {
            $scope.$watch($attrs['ngBind'], function () { return $elem[0].scrollIntoView(true); });
        }
    };
    ...
    return {
        ...
        link: function (scope, element, attrs, ngModel) {
            ...
            ['blur', 'keyup', 'change'].forEach(function (x) {
                element[0].addEventListener(x, function () { return scope.$apply(read); });
            });
            read();
            function read() {
    ...
    function ConversationController($scope, $rootScope, $http, $upload, $state, $stateParams) {
            ...
            if ($state.current.name === "Home.TextConversation") {
                ...
                var loadMessages = function () {
                    ...
                    $http.get($rootScope.RelayUrl + "/text_conversations/" + convId + "/text_messages").success(function (data) {
                        ...
                        $scope.Messages = data.map(function (x) { return new Models.TextMessage(x); });
                        $scope.IsLoadingMessages = false;
                    });
                };
                if ('ActiveConversations' in $scope.$parent) {
                    $scope.Conversation = $scope.$parent.ActiveConversations.first(function (x) { return x.Id === convId; });
                    if ($scope.Conversation.Raw().text_messages) {
                        $scope.Messages = $scope.Conversation.Raw().text_messages.map(function (x) { return new Models.TextMessage(x); });
                ...
                if ('ActiveConversations' in $scope.$parent) {
                    ...
                    $scope.Conversation = $scope.$parent.ActiveConversations.first(function (x) { return x.Receiver.Id === userId; });
                }
                else {
                    $state.go("Home");
                }
        ...
        $scope.LoadHistory = function () {
            ...
            $http.get($rootScope.RelayUrl + "/text_conversations/" + convId + "/text_messages?page=" + (++page)).success(function (data) {
                ...
                data.forEach(function (x) {
                    var idx = $scope.Messages.findFirstIndex(function (msg) { return msg.Id === x.id; });
                    if (idx === -1) {
        ...
        $scope.Send = function () {
            ...
            $upload.upload({
                ...
                fields: !$scope.Conversation.IsNew ? {
...
(function (Controllers) {
    function HomeController($scope, $rootScope, $state, $http, $timeout) {
        ...
        $http.get($rootScope.RelayUrl + "/text_conversations").success(function (data) {
            conversations = data.map(function (x) { return new Models.TextConversation(x); }).filter(function (x) { return !x.IsGroupChat; });
            $scope.ActiveConversations = conversations.slice(0, 10);
        });
        $http.get($rootScope.RelayUrl + "/profiles/0/contacts").success(function (data) {
            $scope.Contacts = data.map(function (x) { return new Models.Profile(x); });
        });
        (function () {
        ...
        $scope.OpenConversation = function (contact) {
            var conv = conversations.first(function (x) { return x.Receiver.Id === contact.Id; });
            if (conv) {
                conv.UpdatedAt = new Date();
                if ($scope.ActiveConversations.first(function (x) { return x.Id === conv.Id; }) === undefined) {
        ...
        $scope.MoveItem = function (from, to) {
            ...
            $timeout(function () { return $scope.ActiveConversations.splice(to, 0, old); });
        };
    }
        ...
        Object.defineProperty(TextConversation.prototype, "Profiles", {
            get: function () {
                if (this._profiles === undefined) {
                    this._profiles = this.Raw().profiles.map(function (x) { return new Models.Profile(x); });
                }
